...
public class SkyScrapper extends Fixed implements IDrawable{
    ...
    public void handleCollision(PlayerHelicopter playerHelicopter) {
        ...
        int skyScrapperNumber = this.sequenceNumber;
        int helicopterLastSkyScrapperHit = playerHelicopter
                                            .getLastSkyScraperReached();

        if(skyScrapperNumber == helicopterLastSkyScrapperHit) {
            playerHelicopter.setSkyScraperReached();
            currentWorld.updateSkyScrapperTextColor(
                    skyScrapperNumber + 1);
        }
        addCollision(playerHelicopter);

        currentWorld.checkIfThereIsWinner(playerHelicopter);
    }
    ...
    public void handleCollision(NonPlayerHelicopter NonplayerHelicopter) {
        ...
        GameWorld currentWorld = super.getGw();
        int skyScrapperNumber = this.sequenceNumber;
        int helicopterLastSkyScrapperHit = NonplayerHelicopter
                .getLastSkyScraperReached();

        if(skyScrapperNumber == helicopterLastSkyScrapperHit) {
            NonplayerHelicopter.setSkyScraperReached();
        }

        addCollision(NonplayerHelicopter);

        currentWorld.checkIfThereIsWinner(NonplayerHelicopter);
    }
}
