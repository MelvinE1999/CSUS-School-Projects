...
public class RefuelingBlimp extends Fixed implements IDrawable{
    ...
    public void handleCollision(PlayerHelicopter playerHelicopter) {
            int newFuelLevel = this.getGasCapacity() +
                    playerHelicopter.getFuelLevel();
            playerHelicopter.setFuelLevel(newFuelLevel);
            this.setColor(ColorUtil.rgb(50, 205, 50), this);
            GameWorld currentWorld = super.getGw();
            GameWorldCollection currentGameObjects = currentWorld
                                                          .getGameObjects();
            if(currentWorld.getSoundToggle())
                currentWorld.playRefuelingSound();

            this.setGasCapacity(0);
            currentGameObjects.remove(this);
            currentGameObjects.add(new RefuelingBlimp());
    }

    @Override
    public void handleCollision(NonPlayerHelicopter nonPlayerHelicopter) {
            int newFuelLevel = this.getGasCapacity() +
                    nonPlayerHelicopter.getFuelLevel();
            nonPlayerHelicopter.setFuelLevel(newFuelLevel);
            this.setColor(ColorUtil.rgb(50, 205, 50), this);
            GameWorld currentWorld = super.getGw();
            GameWorldCollection currentGameObjects = currentWorld
                    .getGameObjects();
